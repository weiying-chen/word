...
def test_generate_subs_strips_trailing_cps_ignore_marker_from_body(
    tmp_path: Path,
) -> None:
    template_path = tmp_path / "template.docx"
    source_docx = tmp_path / "source.docx"
    ...
    input_path.write_text(
        "\n".join(
            [
                ...
                "Looking out over Taipei, I was struck by #",
                "00:01:19:13\t00:01:21:28\t一眼望去",
    ...
    assert "Looking out over Taipei, I was struck by" in texts
...
def test_generate_subs_stops_xxx_highlight_before_next_timestamp_block(
    tmp_path: Path,
) -> None:
    template_path = tmp_path / "template.docx"
    source_docx = tmp_path / "source.docx"
    ...
    assert xxx_marker.runs and all(
        r.font.highlight_color == WD_COLOR_INDEX.YELLOW for r in xxx_marker.runs if r.text
    )
    assert xxx_translation.runs and all(
        r.font.highlight_color == WD_COLOR_INDEX.YELLOW
        for r in xxx_translation.runs
        if r.text
    )
    assert all(r.font.highlight_color is None for r in next_timestamp.runs if r.text)
    assert all(r.font.highlight_color is None for r in next_translation.runs if r.text)
...
def test_generate_subs_highlights_from_xxx_line_after_timestamp_only_line(
    ...
    timestamp = next(p for p in doc.paragraphs if p.text.strip() == "02:40")
    marker = next(p for p in doc.paragraphs if p.text.strip() == "XXX")
    first = next(p for p in doc.paragraphs if p.text.strip() == "1. Good sleep is essential")
...
def test_generate_subs_does_not_highlight_block_when_xxx_is_not_at_line_start(
    ...
    first = next(p for p in doc.paragraphs if p.text.strip() == "This line mentions XXX later.")
    second = next(p for p in doc.paragraphs if p.text.strip() == "Second line stays plain.")
